...
class Log(@volatile var dir: File,
    ...
    try {
      ...
      lock synchronized {
        ...
        if (assignOffsets) {
          ...
          val validateAndOffsetAssignResult: LogValidator.ValidationAndOffsetAssignResult = try {
        ...
        val logOffsetMetadata = LogOffsetMetadata(
         ...
         //调用FileRecords.append()方法写入log文件
  ...
  private def maybeRoll(messagesSize: Int, maxTimestampInMessages: Long, maxOffsetInMessages: Long): LogSegment = {
    val segment: LogSegment = activeSegment
    val now: Long = time.milliseconds
    val reachedRollMs: Boolean = segment.timeWaitedForRoll(now, maxTimestampInMessages) > config.segmentMs - segment.rollJitterMs
